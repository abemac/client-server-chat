...
class RDTSender:
    ...
    def rdt_send(self, bytes, addr):
        ...
        if id not in self.states:
            self.states[id]=0
            print(self.str_time() + "Sender: never sent to this id before")

        if self.states[id]==0:  # State 0 is the for sending packet with seq # 0
            print(self.str_time() + "Sender: State is now 0 for " + id)
            ...
            print(self.str_time() + "Sender: State is now 1 for " + id)
    ...
    def waitACK(self, ack_number, prev_id):
        print(self.str_time() + "Sender: waiting for ACK " + str(ack_number) + " from: " + prev_id)
        while True:
            ...
            id=str(addr)
            print(self.str_time() + "Sender: received ACK from: " + id)
            if id == prev_id and self.isACK(bytes) and received_ack_num == ack_number:
                print(self.str_time() + "Sender: CORRECT ACK " + str(ack_number) + " received")   # The ACK number is the expected one for the current state, accept it
                return
            else:
    ...
    def udt_send(self,bytes,seqnum,addr):
        ...
        print(self.str_time() + "Sender: udt SENDING DATA to id=" + str(addr) + ": " + data.decode())
...
class RDTReceiver:
    ...
    def rdt_recv(self,bufsize):
        while True:
            ...
            if self.states[id] == 0:
                print(self.str_time() + "Receiver: State is now 0 for " + id)
                if seq == 0:                # State 0 is for sending an ACK=0 for a received file
                    print(self.str_time() + "Receiver: CORRECTLY received seq num 0")
                ...
                if seq == 1:
                    print(self.str_time() + "Receiver: CORRECTLY received seq num 1")
    ...
    def ACK(self, ack_number,addr):
        ...
        self.socket.sendto(data,addr)
        print(self.str_time() + "Receiver: Sending ACK " + str(ack_number) + " to id="+str(addr))
